...
function CountUpTimer() {
  ...
  const [results, setResults] = useState(() => JSON.parse(localStorage.getItem('countUpResults')) || []);
        ...
        <h1 className={cx('time')}>{formatTime(time)}</h1>
        {
          isRunning ? (
            <div  className={cx('group_btn')}>
              <button className={cx('btn_stop','btn')}onClick={() => {
                setIsRunning(false);
              }}>
                Dừng
              </button>
              <button className={cx('btn_reset','btn')}onClick={
                handleReset
              }>
                Khởi động lại
              </button>
            ...
            <div className={cx('group_btn')}>
              <button className={cx('btn_start','btn')} onClick={() => {
                if(time == 0) {
              ...
              {time != 0 ? <button className={cx('btn_reset','btn')} onClick={
            handleReset
